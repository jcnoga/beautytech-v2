    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:FB,color:C.text}}>
      ...
      {step !== "confirmado" && (
        <div style={{display:"flex",justifyContent:"center",gap:8,padding:"20px 24px 0"}}>
          {(["data","servico","profissional","dados"] as Step[]).map((s,i) => (
            <div key={s} style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{
                width:28,height:28,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:12,fontWeight:700,
                background: step===s ? accent : ["data","servico","profissional","dados"].indexOf(step) > i ? `${C.green}30` : C.card2,
                color: step===s ? "#fff" : ["data","servico","profissional","dados"].indexOf(step) > i ? C.green : C.textMuted,
                border: `2px solid ${step===s ? accent : ["data","servico","profissional","dados"].indexOf(step) > i ? C.green : C.border}`,
              }}>{i+1}</div>
      ...
      <div style={{maxWidth:560,margin:"0 auto",padding:"24px 16px"}}>
        ...
        {step === "profissional" && (
          <div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {professionals.map(p => (
                <div key={p.id} onClick={() => { setSelPro(p); setStep("dados"); }}
                  style={{
