...
old = """          {(["servico","profissional","data","dados"] as Step[]).map((s,i) => (
            <div key={s} style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:11,color:step===s?accent:C.textMuted,display:"none"}}>
                {["Servi̤o","Profissional","Data","Dados"][i]}
              </span>
...
new = """          {(["data","servico","profissional","horario","dados"] as Step[]).map((s,i) => (
            <div key={s} style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:11,color:step===s?accent:C.textMuted,display:"none"}}>
                {["Data","Servico","Profissional","Horario","Dados"][i]}
              </span>
...
old = """        {/* STEP 1: SERVICO */}
        {step === "servico" && (
          <div>
            <h2 style={{fontSize:20,fontWeight:800,marginBottom:6}}>Escolha o servi̤o</h2>"""
new = """        {/* STEP DATA (novo - primeiro passo) */}
        {step === "data" && (
          <div>
            <h2 style={{fontSize:20,fontWeight:800,marginBottom:6}}>Escolha a data</h2>
            <p style={{color:C.textMuted,fontSize:13,marginBottom:20}}>Quando voce quer ser atendido?</p>
...
new = """        {/* STEP HORARIO (novo) */}
        {step === "horario" && (
          <div>
            <h2 style={{fontSize:20,fontWeight:800,marginBottom:6}}>Escolha o horario</h2>
            <p style={{color:C.textMuted,fontSize:13,marginBottom:20}}>Horarios disponiveis em {selDate}</p>
            <div>
              {(() => {
                const allSlots = [];
                for (let h = 8; h < 18; h++) {
