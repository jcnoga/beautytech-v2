...
export async function prospectModule(fastify: FastifyInstance) {
  ...
  fastify.get("/super-admin/prospects/whatsapp/status", { preHandler: [requireSuperAdmin] }, async (req: any, reply) => {
    ...
    try {
      ...
      const state = inst?.instance?.status ?? inst?.instance?.state ?? inst?.state ?? "";
      const connected = state === "open" || state === "connected";
      const phone = inst?.instance?.owner ?? inst?.instance?.profileName ?? inst?.profileName ?? "";
  ...
  fastify.post("/super-admin/prospects/send-one", { preHandler: [requireSuperAdmin] }, async (req: any, reply) => {
    ...
    // Busca template
    const tmplResult = await db.execute(sql`SELECT * FROM prospect_templates WHERE is_active = true ORDER BY RANDOM() LIMIT 1`) as any;
