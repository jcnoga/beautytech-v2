...
export async function authenticate(req: FastifyRequest, reply: FastifyReply): Promise<void> {
  ...
  try {
    ...
    if (!profile) {
      reply.status(403).send({ success: false, error: "Perfil não encontrado", code: "FORBIDDEN" });
      return;
    }
    if (profile.tenantDeletedAt) {
      reply.status(403).send({ success: false, error: "Salão desativado", code: "TENANT_DELETED" });
      return;
    }
